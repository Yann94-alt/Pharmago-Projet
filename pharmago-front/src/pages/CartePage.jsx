...
import { getNearbyPartnerPharmacies, getNearbyPharmacies } from '../api/api.js'
...
export default function CartePage() {
  ...
  // Source des données : 'garde' -> pharmacies de garde (GardePharmacie)
  //                       'partner' -> pharmacies partenaires (Pharmacies) [valeur par défaut]
  const source = searchParams.get('source') === 'garde' ? 'garde' : 'partner'
  ...
  useEffect(() => {
    ...
    async function loadNearby() {
      try {
        setLoading(true)
        console.log('🔎 Recherche pharmacies autour de :', currentCoords, '| source =', source)
 
        // On choisit le bon endpoint selon la provenance de la pharmacie cliquée :
        // - 'garde'   -> pharmacies de garde (table garde_pharmacies)
        // - 'partner' -> pharmacies partenaires (table pharmacies)
        const fetcher = source === 'garde' ? getNearbyPharmacies : getNearbyPartnerPharmacies
        ...
        if (pharmacyId) {
          ...
          if (found) {
            ...
            console.warn('⚠️ Pharmacie cliquée introuvable dans les résultats. id =', pharmacyId, 'source =', source)
          }
        }
      } catch (error) {
    ...
    loadNearby()
  }, [isNearbyMode, currentCoords?.lat, currentCoords?.lng, pharmacyId, source])
    ...
    <div className="h-screen w-full relative flex flex-col bg-slate-100">
      ...
      {mapCenter ? (
        ...
        >
          ...
          {/* Marqueurs des pharmacies proches (garde ou partenaires selon la source) */}
      ...
      {isNearbyMode && !loading && pharmacies.length === 0 && !locationError && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1000] bg-white px-4 py-2 rounded-xl shadow-md border text-xs font-medium text-slate-600">
          Aucune pharmacie {source === 'garde' ? 'de garde' : 'partenaire'} à proximité
        </div>
      )}
          ...
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[10px] font-bold tracking-wider text-emerald-600 uppercase bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">
                {source === 'garde' ? 'Pharmacie de Garde' : 'Pharmacie Partenaire'}
              </span>
